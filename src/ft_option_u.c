...
/*   Updated: 2018/11/16 06:41:10 by rgermain    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static int	ft_option_neg(t_option *lst, unsigned int nb, int count)
{
	...
	if (lst->sign == '+')
		pos = 1;
	len = ft_uintlen(nb) + pos;
	if (lst->af_nb > ft_uintlen(nb))
		count += ft_print_prefix(ft_uintlen(nb), lst->af_nb, 1);
	ft_putunbr(nb);
	if (len < lst->af_nb)
...
static int	ft_option_pos(t_option *lst, unsigned int nb, int count)
{
	...
	if (len < lst->af_nb)
		...
		count += ft_print_prefix(len, lst->bf_nb, 0);
	if (lst->af_nb > ft_uintlen(nb))
		count += ft_print_prefix(ft_uintlen(nb), lst->af_nb, lst->point);
	ft_putunbr(nb);
	return (count + len);
}
