...
/*   Updated: 2018/11/24 12:14:11 by rgermain    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int			ft_option_x_maj(t_valst *lst_va, char *str, int count, int index)
{
	...
	if (lst->point == 0 && lst->hash == 1 && (index > 2 || nb != 0))
		len += ft_putstr("0X");
	if ((lst->sign != '-') &&
...
{
	tab_i[1] += ft_option_x_maj(lst_va, str, tab_i[0], index);
